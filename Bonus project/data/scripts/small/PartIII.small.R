...
meta$class <- ifelse(meta$label == "HD", "Healthy", "Cancer")
...
write.table(
    pvalue_df,
    "/root/exSEEK/result/P3/small/filter.pvalues.all.txt",
    sep = "\t",
    quote = FALSE,
    row.names = FALSE
)

feature_counts <- c(1:10, 20, 30, 50, 100, 150, 200, 300, 500)
feature_lists <- list()
...
write.xlsx(feature_lists, "/root/exSEEK/result/P3/small/feature.lists.xlsx")
...
pdf("/root/exSEEK/result/P3/small/AUC.v.features.pdf", width = 8, height = 6)
...
top20_genes <- names(p_values_sorted[1:20])

X_train_top <- X_train[, top20_genes]
X_test_top <- X_test[, top20_genes]
...
pdf("/root/exSEEK/result/P3/small/ROC.curve.pdf", width = 6, height = 6)
...
expr_heat <- logcpm_matrix[top20_genes, rownames(X_test)]
...
pdf("/root/exSEEK/result/P3/small/heatmap.top20.pdf", width = 10, height = 8)
pheatmap(
    expr_heat,
    scale = "column",
    ...
    show_rownames = FALSE,
    show_colnames = FALSE,
    main = "Top 20 Genes - Healthy vs Cancer (Grouped by Class)",
    fontsize = 10,
    cluster_rows = FALSE,
    cluster_cols = TRUE
)
